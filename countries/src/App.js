...
const useCountry = (name) => {
  ...
  useEffect( async () => {
    ...
    if (name) {
      try {
        const search = await axios.get(`https://restcountries.eu/rest/v2/name/${name}?fullText=true`)
        // .then(search => setCountry({ ...search, found: true}))
        setCountry({ ...search, found: true})
      } catch {
        console.error("didn\'t find country")
